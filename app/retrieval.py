...
def get_chunk_id(chunk: dict) -> str:
    # Use chunk_id if available (diabetes chunks), otherwise reconstruct from pmid/index
...
class Retriever:
    ...
    def __init__(self, config_path: str = "config.yaml", strategy: str = "fixed"):
    ...
    def _rerank(self, query: str, chunks: list[dict], top_k: int) -> list[dict]:
        ...
        try:
            ...
            pairs = [(query, c.get("abstract", c.get("_text", ""))[:512])
                     for c in chunks]
    ...
    def retrieve(self, query: str, method: str = "hybrid", top_k: int = 5,
        ...
        Returns:
            ...
            Each chunk has: chunk_id, title, abstract, authors, year, journal
        """
        self._assert_loaded()
    ...
    def format_context(self, chunks: list[dict], max_chars: int = 4000) -> str:
        ...
        Format: Authors (Year) - inline academic citation style
        Each block is a clickable citation in the UI.
        """
        parts = []
        total = 0
        for c in chunks:
            ...
            # Use Authors (Year) format - inline citation style, matches app.py display for clickability
            ...
            if total + len(block) > max_chars:
                ...
                if remaining > 100:
                    abstract_truncated = abstract[:remaining] + "..."
                    block = f"{header}\n\n{abstract_truncated}"
                    parts.append(block)
                break
