...
def main() -> None:
    ...
    log.info("=" * 60)
    log.info("Diabetes RAG - BM25 Index Builder")
    ...
    # Quick sanity check
    log.info("Sanity check - querying 'diabetes insulin treatment':")
    test_tokens = tokenize("diabetes insulin treatment")
    ...
    for idx in top_idx:
        ...
        log.info(
            "  [%.3f] %s - %s",
            scores[idx], c.get("pmid", ""), c.get("title", "")[:60]
        )

    log.info("Done. Next step: python pipeline/embed_and_upsert.py --strategy fixed")
